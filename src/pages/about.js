...
const AboutTimeline = ({ entries }) => {
  return (
    <AboutOuter>
      {Object.entries(entries).map(([year, record]) => (
        <AboutYearRow key={year}>
          <AboutYear>
            <span>{year}</span>
          </AboutYear>
          {record.map((item, idx) => (
...
const FounderCard = styled.div`
  ...
  .description {
   ...
   &:first-of-type:hover, &:nth-of-type(3n):hover {
    &>div {
      background: #1b5e59;
    }
   }
  ...
  &>div {
    ...
    background: linear-gradient(360deg, rgba(0, 0, 0, 0.57) 3.47%, rgba(255, 255, 255, 0) 100%);
  }
  .description {
    opacity: 0;
  }
...
const FounderName = styled.p`
  font-weight: 600;
`
const FounderPosition = styled.p`
  font-weight: 300;
`
...
const AboutPage = props => {
  ...
  // TODO: use EID's for this:
  const content = props.data.allContentfulHomepage.edges[0].node
  const aboutContent = props.data.allContentfulAboutPage.edges[0].node
  const events = aboutContent.events.reduce((obj, item) => {
    return ({
      ...obj,
      [item.year]: obj[item.year] ? [...obj[item.year], {
        description: item.description,
        position: item.alignRight ? 'right' : 'left',
      }] : [{
        description: item.description,
        position: item.alignRight ? 'right' : 'left',
      }],
              ...
              <FlexWrapper wrap>
                {aboutContent.founders.map((item, idx) => (
                  <FounderCard key={idx} bg={item.portrait.file.url}>
                    <div>
                      <FounderDescription className="description">
